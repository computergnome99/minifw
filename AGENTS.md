...
MiniFW is a Bun-only TypeScript library for server-rendered HTMX applications. Its public API is split into two package subpaths:

- `minifw/core`: `mini`, `page`, `partial`, `layout`, `fragment`, and shared public types.
- `minifw/helpers`: HTML/CSS template tags and small rendering helpers.

Source lives under `lib/`. Keep public API in `lib/core/` or `lib/helpers/`; `lib/internal/` and `lib/runtime/` are implementation details and must not be exported as package subpaths.
...
| Testing                     | Adding, changing, debugging, or reviewing Bun specs and fixtures.                                      | [.ai/skills/testing/SKILL.md](.ai/skills/testing/SKILL.md)             |
